...
class ConfiguracionRequisicion extends ConfiguracionRequisicionPolicy
{
    ...
    public function consultarPerfiles()
    {
        $query = "SELECT    P.nombre AS 'perfiles.nombre', SE.descripcion AS 'servicio_estatus.descripcion' , CPSE.*
                FROM        configuracion_perfil_estatus CPSE
                INNER JOIN  perfiles P ON CPSE.perfilId = P.id
                INNER JOIN  estatus SE ON CPSE.EstatusId = SE.id
    ...
    public function consultarFlujo()
    {
        $query = "SELECT    SE.descripcion AS 'estatus.descripcion',
                ...
                INNER JOIN  estatus SE ON CF.estatusId = SE.id
                INNER JOIN  estatus SSE ON CF.siguienteEstatusId = SSE.id
    ...
    function eliminarPerfiles()
    {
        ...
        return Conexion::queryExecute($this->bdName, "DELETE FROM configuracion_perfil_estatus WHERE configuracionRequisicionId = :configuracionRequisicionId AND documentoTipo = :documentoTipo", $eliminar, $eliminarPDOParam, $error);
    }
        ...
        if ( $arrayPerfiles ) {
            ...
            foreach ($arrayPerfiles as $perfil => $perfilEstatus) {
                ...
                foreach ($perfilEstatus as $Estatus => $value) {
                    ...
                    $respuesta = Conexion::queryExecute($this->bdName, "INSERT INTO configuracion_perfil_estatus (configuracionRequisicionId, documentoTipo, perfilId, EstatusId, modificar, automatico, usuarioIdCreacion) VALUES (:configuracionRequisicionId, :documentoTipo, (SELECT id FROM perfiles WHERE nombre = :perfil), (SELECT id FROM estatus WHERE descripcion = :EstatusId), :modificar, :automatico, :usuarioIdCreacion)", $insertar, $insertarPDOParam, $error);
        ...
        if ( $arrayFlujo ) {
            ...
            foreach ($arrayFlujo as $estatus => $siguienteEstatus) {
                ...
                foreach ($siguienteEstatus as $Estatus) {
                    ...
                    $respuesta = Conexion::queryExecute($this->bdName, "INSERT INTO configuracion_flujo (configuracionRequisicionId, documentoTipo, estatusId, siguienteEstatusId, usuarioIdCreacion) VALUES (:configuracionRequisicionId, :documentoTipo, (SELECT id FROM estatus WHERE descripcion = :estatus), (SELECT id FROM estatus WHERE descripcion = :siguienteEstatus), :usuarioIdCreacion)", $insertar, $insertarPDOParam, $error);
                }
