...
class SaveGenerosRequest extends Request
{
    static public function rules($id)
    {
        if ( self::method() === 'POST' ) {
            $rules = [ 'descripcion' => 'required|string|max:60|unique:'.CONST_BD_APP.'.generos', 
                       'nombreCorto' => 'string|max:20' ];
        } else {
            $rules = [ 'descripcion' => 'required|string|max:60|unique:'.CONST_BD_APP.'.generos:id:' . $id, 
                       'nombreCorto' => 'string|max:20' ];
        }
    ...
    static public function validated($id = null) {
        return self::validating(Genero::fillable(), self::rules($id), self::messages());
    }
}
