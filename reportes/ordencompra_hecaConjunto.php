...
function generarPDFOrdenes($datos) {
    ...
    $pdf->SetFont('helvetica', '', 7.5); // Fuente, Tipo y Tamaño
    $pdf->MultiCell(0, 3, "{$orden["folio"]}", 'B', 'C', 0, 1, '', '', true);

    $fecha = fFechaLarga($orden["fechaRequerida"]);
    $fechaCreacion = fFechaLarga($orden["fechaCreacion"]);
    ...
    $pdf->SetFont('helvetica', '', 8); // Fuente, Tipo y Tamaño
    $subtotal = 0;
    foreach($orden["partidas"] as $key => $detalle) {
    ...
    $pdf->MultiCell(0, 5, $orden["subtotal"], 0, 'R', 0, 1, '', '', true, 0, false, true, '5', 'M');
    ...
    if ( $y > 260 ) {
        ...
        $y = 40; // Reiniciar la posición Y
    }

    $pdf->SetFont('helvetica', 'B', 8); // Fuente, Tipo y Tamaño
    $pdf->MultiCell(25, 20, "FACTURAR A:", 0, '', 0, 0, '10', $y, true, 0, false, true, '5', 'M');
    $pdf->SetFont('helvetica', '', 8); // Fuente, Tipo y Tamaño
    ...
    $pdf->MultiCell(66, 5, "AUTORIZA", 0, 'C', 0, 1, '', '', true);
        ...
        $fileName = "orden_compra_{$orden['folio']}_" . time() . ".pdf";
