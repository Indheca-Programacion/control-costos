...
$pdf->folio = mb_strtoupper(fString($ordenCompraGlobal->id));
$pdf->fechaCreacion = $ordenCompraGlobal->fechaCreacion;

// set document information
$pdf->setTitle("OC {$ordenCompraGlobal->folio}");
...
$fecha = strtotime($ordenCompraGlobal->fechaCreacion);
...
$almacen = mb_strtoupper(fString($ordenCompraGlobal->almacen_obra ?? '109'));

$folioOC = $ordenCompraGlobal->reposicion_gastos == 1 ? "RG".$ordenCompraGlobal->folio : $ordenCompraGlobal->folio;
$pdf->MultiCell(35, 3, "FOLIO OC", 0, 'R', 0, 0, 5, '', true);
$pdf->MultiCell(45, 3, "{$folioOC}", 'B', 'C', 0, 0, '', '', true);

$pdf->MultiCell(35, 3, "FOLIO RQ", 0, 'R', 0, 0, 120, '', true);
$pdf->MultiCell(45, 3, "{$ordenCompraGlobal->REQUISICIONES}", 'B', 'C', 0, 1, '', '', true);
...
$fecha = fFechaLarga($ordenCompraGlobal->fechaRequerida);
$razon = mb_strtoupper($proveedor->razonSocial ?? '');
...
$pdf->MultiCell(167, 3, mb_strtoupper("$proveedor->domicilio"), 'B', 'L', 0, 1, '', '', true);
...
$subtotal = 0;
foreach($ordenCompraGlobal->detalles as $key => $detalle) {
	...
	$pu = number_format($detalle['importeUnitario'],2);
	$costo = number_format(($detalle['importeUnitario']*$detalle['cantidad']),2) ;
	...
	$num_detalle = count($ordenCompraGlobal->detalles);
	...
	$subtotal += $detalle['importeUnitario']*$detalle['cantidad'];
...
$pdf->MultiCell(10, 5, $ordenCompraGlobal->retencionIva, 0, 'C', 1, 0, '', '', true, 0, false, true, '5', 'M');

$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(33, 5, "Retención I.S.R.:", 0, 'R', 1, 0, '73', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 0, 0); // Set text color to black
$pdf->MultiCell(40, 5, ($ordenCompraGlobal->retencionIsr??0), 0, 'L', 1, 0, '', '', true, 0, false, true, '5', 'M');

$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(32, 5, "Subtotal", 0, 'L', 1, 0, '136', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->MultiCell(32, 5, "{$ordenCompraGlobal->subtotal}", 0, 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(143, 5, "Descuentos", 0, 'R', 1, 0, '', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->MultiCell(0, 5, "{$ordenCompraGlobal->descuento}", 0, 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');
$pdf->SetFont('helvetica', 'B', 8); // Fuente, Tipo y Tamaño
...
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->MultiCell(0, 5, $ordenCompraGlobal->iva, 0, 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');

$totalEnLetra = numeroALetras($ordenCompraGlobal->total, $divisa->descripcion, $divisa->nombreCorto);
...
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->SetFont('helvetica', 'B', 8); // Fuente, Tipo y Tamaño
$total = formatMoney($ordenCompraGlobal->total);
$pdf->MultiCell(30, 5, "$ordenCompraGlobal->total", 'B', 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');
...
$especificaciones = mb_strtoupper(fString($ordenCompraGlobal->especificaciones ?? ''));
...
$direccionEntrega = mb_strtoupper(fString($ordenCompraGlobal->direccion ?? ''));
...
switch ($ordenCompraGlobal->condicionPagoId) {
	case 1:
	...
	case 4:
		...
		break;
	case 5:
		$condicionPago = 'OTRO - ANTICIPO A CONTRA ENTREGA';
		break;
	default:
		$condicionPago = '';
		break;
}
$condicionPago = mb_strtoupper(fString($condicionPago));
$pdf->SetLineWidth(0.2); // Reset line width to default
$pdf->MultiCell(0, 5, "$condicionPago", 'B', '', 1, 1, '', '', true, 0, false, true, '5', 'M');

$pdf->SetLineWidth(0.2); // Reset line width to default

$justificacion = mb_strtoupper(fString($ordenCompraGlobal->justificacion ?? ''));
...
$tipoRequisicion = mb_strtoupper(fString($ordenCompraGlobal->tipoRequisicion ?? 0));
...
$pdf->MultiCell(100, 12, "$tipoRequisicion \n $ordenCompraGlobal->tiempoEntrega", 0, 'C', 0, 1, 100, '', true, 0, false, true, '12', 'M');
...
$contrato = mb_strtoupper(fString(!is_null($ordenCompraGlobal->descripcion_obra) ? $ordenCompraGlobal->descripcion_obra : '109. mantenimiento correctivo general'));
$yRect = $pdf->getY();
...
$pdf->Output("OC {$ordenCompraGlobal->folio}.pdf", 'I');
