...
$folioOC = $ordenCompra->reposicion_gastos == 1 ? "RG".$ordenCompra->folio : $ordenCompra->folio;
...
$pdf->MultiCell(10, 5, $ordenCompra->retencionIva, 0, 'C', 1, 0, '', '', true, 0, false, true, '5', 'M');

$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(33, 5, "Retención I.S.R.:", 0, 'R', 1, 0, '73', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 0, 0); // Set text color to black
$pdf->MultiCell(40, 5, ($ordenCompra->retencionIsr??0), 0, 'L', 1, 0, '', '', true, 0, false, true, '5', 'M');

$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(32, 5, "Subtotal", 0, 'L', 1, 0, '136', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->MultiCell(32, 5, $ordenCompra->subtotal, 0, 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');
$pdf->SetTextColor(0, 128, 255); // Set text color to blue
$pdf->MultiCell(143, 5, "Descuentos", 0, 'R', 1, 0, '', '', true, 0, false, true, '5', 'M');
$descuentos = floatval($ordenCompra->descuento ?? 0);
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
...
$pdf->SetTextColor(0, 0, 0); // Set text color to blue
$pdf->MultiCell(0, 5, $ordenCompra->iva, 0, 'R', 1, 1, '', '', true, 0, false, true, '5', 'M');
...
$especificaciones = mb_strtoupper(fString($ordenCompra->especificaciones ?? ''));
...
$direccionEntrega = mb_strtoupper(fString($ordenCompra->direccion ?? ''));
...
$justificacion = mb_strtoupper(fString($ordenCompra->justificacion ?? ''));
...
$pdf->MultiCell(100, 12, "$tipoRequisicion \n $ordenCompra->tiempoEntrega", 0, 'C', 0, 1, 100, '', true, 0, false, true, '12', 'M');
...
// CUADRO PARA LOS DATOS DE FACTURA Y DATOS BANCARIOS
$y = $pdf->getY() + 3.6;
$x = 9;

if (!empty($datosBancariosProveedor) && is_array($datosBancariosProveedor)) {
    if (count($datosBancariosProveedor) > 4) {
		$y = $pdf->getY() + 19;
    }
}
...
$anchoCuadro = 191; 
$altoCuadro  = 25;
$pdf->SetFillColor(239, 248, 255); 
$pdf->Rect($x, $y, $anchoCuadro, $altoCuadro, 'F'); 
...
$pdf->SetFont('helvetica', 'B', 8); // Fuente, Tipo y Tamaño
$pdf->MultiCell(0, 5, "No de Cta del Proveedor:", 0, '', 1, 1, 76, '', true, 0, false, true, '5', 'M');
$pdf->SetFont('helvetica', '', 6.5); // Fuente, Tipo y Tamaño
...
foreach($datosBancariosProveedor as $index => $value){
    $nombreBanco = mb_strtoupper(fString($value["nombreBanco"] ?? ''));
    $cuenta      = mb_strtoupper(fString($value["cuenta"] ?? ''));
    $clabe       = mb_strtoupper(fString($value["cuentaClave"] ?? ''));
    ...
    // Incrementar X solo después de dibujar la celda
    if (($index + 1) % 4 == 0) {
        // cada 3 celdas, bajar Y y reiniciar X
        $y += 17;
        $x = $xInicial;

		
    } else {
        $x += 30; // ancho de celda + espacio
    }
}
...
if ( isset($autorizacionAdicional) ) {
	$pdf->addPage(); // Agregar nueva página si se excede el límite
    $pdf->MultiCell(66, 5, "AUTORIZACION ADICIONAL", 0, 'C', 0, 1, '', '', true);
	$pdf->ln(10); // Salto de Línea
	$pdf->Image($autorizacionAdicionalFirma, 10, 35, 66, 0, 'JPG', '', '', false, 300, '', false, false, 0, 'CT', false, false);
    $pdf->MultiCell(66, 5, "{$autorizacionAdicional}", 'T', 'C', 0, 1, '', '', true);
	$pdf->MultiCell(66, 5, "{$autorizacionAdicionalPuesto}", 0, 'C', 0, 1, '', '', true);
	$pdf->MultiCell(66, 5, "{$autorizacionAdicionalEmpresa}", 0, 'C', 0, 1, '', '', true);
}
